...
4. Frontend shows the photo in the map surface and enables analysis when the backend is ready.
5. User starts skin analysis.
6. Frontend sends `file`, `goal_id=skin-health-analysis`, and `parameters={}` to `POST /api/process`.
7. Backend validates multipart input, JSON parameter shape, content type, file size, and goal ID.
8. Backend decodes the image with OpenCV-Python.
9. Backend detects a face with Haar cascade when available and uses a centered fallback region when needed.
10. Backend normalizes lighting, estimates a skin mask, splits the face into forehead, cheeks, nose, and chin, and measures acne, dark spots, wrinkles, redness, and pores.
11. Backend returns an overlay PNG plus `X-DermaScope-Analysis` and processing metadata headers.
12. Frontend ignores stale responses, parses metadata safely, and displays the overlay, Skin Health Score, condition rows, and zone sectors.
...
3. `POST /api/process` validates and processes one image in memory.
4. API errors return safe `{ code, message }` JSON.
5. The backend does not persist uploads, derived overlays, or analysis metadata.
...
2. Test backend processing returns PNG overlay and metadata for a valid sample image.
3. Test backend rejects unknown goals, wrong file types, empty files, oversized files, and invalid parameter JSON.
4. Test frontend build and lint.
5. Manually test upload, camera capture, service-offline state, successful analysis, fallback warning, reset, and mobile recomposition with real face photos.
